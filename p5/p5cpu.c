...
void* getWordCountsCPU(){
	...
	ENTRY item;
	ENTRY *found_item;
//printf("\nim in CPU counter : %s",filenameCPU);
	if(strstr(filenameCPU,"1")!=NULL) id=1;
	if(strstr(filenameCPU,"2")!=NULL) id=2;
//	myprintf(filenameCPU,0);
//	pintf("\nFile Read..");
//	pintf(filenameCPU);
//do{
	cptr = NULL;result = NULL;
	fp = fopen(filenameCPU,"r");
	if(fp == NULL) {
	        perror("failed to open ");perror(filenameCPU);
        	return ;//EXIT_FAILURE;
	}
...
void init(){
	...
	//filenameCPU = (char*) malloc (LINE_MAX * sizeof(char));
	hcreate(MAX_WORDS);
...
//		pintf(filenameCPU);
		...
		pthread_create(&p[pi],NULL,(void *)&getWordCounts,(void*)filenameCPU);
		pi++;
		task_threads++;
...
void create_thread(){
//	printf("Im in create thread and im passing %s",filenameCPU);
		pthread_create(&cpu,NULL,(void *)&getWordCountsCPU,NULL);
}
